...
public class GraphPanelChart extends JComponent implements ClipboardOwner {
	...
	private CustomNumberRenderer nbFormatter = new CustomNumberRenderer(
			"#,#00.#", ' ');
	...
	private void autoZoom_orig() {
		...
		Iterator<Entry<String, AbstractGraphRow>> it = rows.entrySet()
				.iterator();
		while (it.hasNext()) {
			Entry<String, AbstractGraphRow> row = it.next();
			double[] minMax = row.getValue().getMinMaxY(
					chartSettings.getMaxPointPerRow());
			if (minMax[1] > 0) {
				double zoomFactor = 1;
	...
	public GraphPanelChart(boolean allowCsvExport, boolean haveGUI) {
		...
		cSplineRowPlotter = new CSplineRowPlotter(chartSettings,
				yAxisLabelRenderer);
	}
	...
	private boolean drawMessages(Graphics2D g) {
		if (errorMessage != null) {
			...
			g.drawString(errorMessage, g.getClipBounds().width / 2
					- g.getFontMetrics(g.getFont()).stringWidth(errorMessage)
					/ 2, g.getClipBounds().height / 2);
			return true;
		}
		if (rows.isEmpty()) {
	...
	private void getMinMaxDataValues() {
		...
		while (it.hasNext()) {
			...
			rowValue.setExcludeOutOfRangeValues(chartSettings
			...
			double[] rowMinMaxY = rowValue.getMinMaxY(chartSettings
					.getMaxPointPerRow());
		...
		if (barValue > 0) {
			maxXVal += barValue;
			// find nice X steps
			double barPerSquare = (double) (maxXVal - minXVal)
					/ (barValue * gridLinesCount);
		...
		if (maxXVal == 0L || maxYVal == 0L || minXVal == Long.MAX_VALUE
				|| minYVal == Double.MAX_VALUE) {
	...
	private void computeChartSteps() {
		...
		// check if step is still good with minY trimmed. If not, use next
		// factor.
		...
		while ((minYVal - minYVal % trim) + foundStep * gridLinesCount >= maxYVal
				&& minYVal > 0) {
	...
	private void setDefaultDimensions(Graphics g) {
		chartRect.setBounds(spacing, spacing, g.getClipBounds().width - spacing
				* 2, g.getClipBounds().height - spacing * 2);
	...
	private int getYLabelsMaxWidth(FontMetrics fm) {
		...
		for (int i = 0; i <= gridLinesCount; i++) {
			yAxisLabelRenderer.setValue((minYVal * gridLinesCount + i
					* (maxYVal - minYVal))
					/ gridLinesCount);
	...
	private void calculateYAxisDimensions(Graphics g) {
		...
		yAxisRect.setBounds(chartRect.x, chartRect.y, axisWidth,
				chartRect.height);
		if (!isPreview) {
			chartRect.setBounds(chartRect.x + axisWidth, chartRect.y,
					chartRect.width - axisWidth, chartRect.height);
		} else {
			chartRect.setBounds(chartRect.x + previewInset, chartRect.y,
	...
	private void calculateXAxisDimensions(Graphics g) {
		...
		xAxisRect.setBounds(chartRect.x, chartRect.y + chartRect.height
				- axisHeight, chartRect.width, axisHeight);
		if (!isPreview) {
			chartRect.setBounds(chartRect.x, chartRect.y, chartRect.width
					- axisEndSpace, chartRect.height - axisHeight);
		} else {
			chartRect.setBounds(chartRect.x, chartRect.y, chartRect.width - 2
					* previewInset, chartRect.height - (previewInset + 1)); // +1
																			// because
																			// layout
																			// take
																			// one
		...
		yAxisRect.setBounds(yAxisRect.x, yAxisRect.y, yAxisRect.width,
				chartRect.height);
	}
	...
	public void paintComponent(Graphics g) {
		...
		if (!cacheValid) {
			if (cache == null || cacheHeight != height || cacheWitdh != witdh) {
				cache = new BufferedImage(witdh, height,
	...
	private void drawPanel(Graphics2D g, boolean drawHoverInfo) {
		...
		if (chartSettings.isDrawGradient()) {
			GradientPaint gdp = new GradientPaint(0, 0, Color.white, 0,
					g.getClipBounds().height, gradientColor);
			g.setPaint(gdp);
		}
	...
	private void paintLegend(Graphics g) {
		...
		while (it.hasNext()) {
			row = it.next();
			// Color color = reSetColors ? colors.getNextColor() :
			// row.getValue().getColor();
			Color color = row.getValue().getColor() != null ? row.getValue()
					.getColor() : colors.getNextColor();

			if (!row.getValue().isShowInLegend()
					|| !row.getValue().isDrawOnChart()) {
				continue;
			}

			String rowLabel = row.getKey();
			if (chartSettings.isExpendRows()
					&& rowsZoomFactor.get(row.getKey()) != null) {
				double zoomFactor = rowsZoomFactor.get(row.getKey());
				if (zoomFactor != 1) {
					if (zoomFactor > 1) {
						int iZoomFactor = (int) zoomFactor;
						rowLabel = rowLabel + " (x"
								+ getNiceNumber(iZoomFactor) + ")";
					} else {
			...
			if (currentX + rectW + spacing / 2 + fm.stringWidth(rowLabel) > g
					.getClipBounds().width) {
			...
			boolean isBarChart = row.getValue().isDrawBar()
					&& chartSettings.getChartType() == ChartSettings.CHART_TYPE_DEFAULT
			...
			Rectangle r = new Rectangle(currentX + legendAdjust + 1, currentY
					+ legendAdjust + 1, rectW - 2 * legendAdjust, rectH - 2
					* legendAdjust);
			((Graphics2D) g).fill(r);
			row.getValue().setLegendColorBox(r);

			// g.fillRect(currentX+legendAdjust+1, currentY+legendAdjust+1,
			// rectW-2*legendAdjust, rectH-2*legendAdjust);
			if (isBarChart) {
				((Graphics2D) g).setComposite(oldComposite);
			}
			g.setColor(color.darker());
			g.drawRect(currentX + legendAdjust + 1,
					currentY + legendAdjust + 1, rectW - 2 * legendAdjust,
					rectH - 2 * legendAdjust);
		...
		legendRect.setBounds(chartRect.x, chartRect.y, chartRect.width,
				legendHeight);
		chartRect.setBounds(chartRect.x, chartRect.y + legendHeight + spacing,
				chartRect.width, chartRect.height - legendHeight - spacing);
	}
	...
	private void paintYAxis(Graphics g) {
		...
		for (int n = 0; n <= gridLinesCount; n++) {
			gridLineY = chartRect.y
					+ (int) ((gridLinesCount - n) * (double) chartRect.height / gridLinesCount);
		...
		for (int n = 0; n <= gridLinesCount; n++) {
			...
			g.setColor(axisColor);
			g.drawLine(chartRect.x + shift, gridLineY, chartRect.x
					+ chartRect.width, gridLineY);
			g.setColor(Color.black);

			// draw label
			if (!isPreview) {
				yAxisLabelRenderer.setValue((minYVal * gridLinesCount + n
						* (maxYVal - minYVal))
						/ gridLinesCount);
				valueLabel = yAxisLabelRenderer.getText();
				labelXPos = yAxisRect.x + yAxisRect.width
						- fm.stringWidth(valueLabel) - spacing - spacing / 2;
				g.drawString(valueLabel, labelXPos, gridLineY + fm.getAscent()
						/ 2);
			}
		}
		...
		if (!isPreview) {
			...
			g.drawString(yAxisLabel, yAxisRect.x + 15, yAxisRect.y
					+ yAxisRect.height / 2 + delta / 2);
	...
	private void paintXAxis(Graphics g) {
		...
		// draw markers
		for (int n = 0; n <= gridLinesCount; n++) {
			gridLineX = chartRect.x
					+ (int) (n * ((double) chartRect.width / gridLinesCount));
			g.drawLine(gridLineX, chartRect.y + chartRect.height - 3,
					gridLineX, chartRect.y + chartRect.height + 3);
		}
		...
		for (int n = 0; n <= gridLinesCount; n++) {
			...
			// draw grid line with tick
			g.setColor(axisColor);
			g.drawLine(gridLineX, chartRect.y + chartRect.height - shift,
					gridLineX, chartRect.y);
			g.setColor(Color.black);

			// draw label - keep decimal precision if range is too small
			double labelValue = minXVal + n * (double) (maxXVal - minXVal)
					/ gridLinesCount;
			...
			g.drawString(valueLabel, labelXPos, xAxisRect.y + fm.getAscent()
					+ spacing);
		}
		...
		// axis label
		g.drawString(getXAxisLabel(), chartRect.x + chartRect.width / 2
				- g.getFontMetrics(g.getFont()).stringWidth(getXAxisLabel())
				/ 2, xAxisRect.y + 2 * fm.getAscent() + spacing + 3);
		...
		if (chartSettings.isDrawCurrentX()) {
			gridLineX = chartRect.x
					+ (int) ((currentXVal - minXVal) * (double) chartRect.width / (maxXVal - minXVal));
			g.setColor(Color.GRAY);
			g.drawLine(gridLineX, chartRect.y, gridLineX, chartRect.y
					+ chartRect.height);
			g.setColor(Color.black);
		}
	}
	...
	private void paintChart(Graphics g) {
		...
		if (chartSettings.getMaxPointPerRow() > 0) {
			...
			while (it.hasNext()) {
				Entry<String, AbstractGraphRow> row = it.next();
				int rowFactor = (int) Math.floor(row.getValue().size()
						/ (double) chartSettings.getMaxPointPerRow()) + 1;
		...
		// paint rows in 2 phases. Raws with draw label are drawn after to have
		// label on top
		it = rows.entrySet().iterator();
	...
	private void paintRows(Graphics g, ColorsDispatcher dispatcher,
			Iterator<Entry<String, AbstractGraphRow>> it, boolean rowsWithLabel) {
		while (it.hasNext()) {
			Entry<String, AbstractGraphRow> row = it.next();
			if (row.getValue().isDrawOnChart()
					&& row.getValue().isDrawValueLabel() == rowsWithLabel) {
				// eto Color color = reSetColors ? dispatcher.getNextColor() :
				// row.getValue().getColor();
				Color color = row.getValue().getColor() != null ? row
						.getValue().getColor() : dispatcher.getNextColor();
				paintRow(g, row.getValue(), row.getKey(), color);
			}
		}
	}

	private void paintRow(Graphics g, AbstractGraphRow row, String rowLabel,
			Color color) {
		...
		if (currentPlotter != null) {
			double zoomFactor = 1;
			if (chartSettings.isExpendRows()
					&& rowsZoomFactor.get(rowLabel) != null) {
				zoomFactor = rowsZoomFactor.get(rowLabel);
			}
			currentPlotter.setBoundsValues(chartRect, minXVal, maxXVal,
					minYVal, maxYVal);
	...
	/**
	 * @param yAxisLabelRenderer
	 *            the yAxisLabelRenderer to set
	 */
	...
	/**
	 * @param xAxisLabelRenderer
	 *            the xAxisLabelRenderer to set
	 */
	...
	private void paintAd(Graphics2D g) {
		...
		g.drawString(AD_TEXT, g.getClipBounds().width
				- g.getFontMetrics().stringWidth(AD_TEXT) - spacing, g
				.getFontMetrics().getHeight() - spacing + 1);
	...
	private void registerPopup(boolean allowCsvExport) {
		...
		itemCopy.setIcon(new javax.swing.ImageIcon(getClass().getResource(
				"copy.png")));
		JMenuItem itemSave = new JMenuItem("Save Image as...");
		itemSave.setIcon(new javax.swing.ImageIcon(getClass().getResource(
				"save.png")));
		JMenuItem itemExport = new JMenuItem("Export to CSV...");
		itemExport.setIcon(new javax.swing.ImageIcon(getClass().getResource(
				"export.png")));
	...
	private synchronized void showHoverInfo() {
		...
		if (isPreview || chartRect.width == 0 || chartRect.height == 0
				|| xHoverInfo == -1 || yHoverInfo == -1) {
			return;
		}

		long realX = minXVal + (maxXVal - minXVal) * (xHoverInfo - chartRect.x)
				/ chartRect.width;
		double realY = minYVal + (maxYVal - minYVal)
				* (chartRect.height - yHoverInfo + chartRect.y)
				/ chartRect.height;
		...
		if (hoverWindow.getWidth() < labelWidth
				|| hoverWindow.getHeight() < labelHeight) {
		...
		double deltaX = Math.abs(hoverWindow.getLocation().getX()
				- hoverWindowX);
		double deltaY = Math.abs(hoverWindow.getLocation().getY()
				- hoverWindowY);

		if (forceHoverPosition || deltaX >= hoverGap || deltaY >= hoverGap) {
			// prevent out of screen
			int correctedX = Math.min(hoverWindowX, Toolkit.getDefaultToolkit()
					.getScreenSize().width - hoverWindow.getSize().width);
	...
	private void chartMouseMoved(java.awt.event.MouseEvent evt) {
		...
		if (x >= chartRect.x && x <= (chartRect.x + chartRect.width)
				&& y >= chartRect.y && y <= (chartRect.y + chartRect.height)) {
			xHoverInfo = x;
			yHoverInfo = y;
			showHoverInfo();
	...
	public void saveGraphToCSV(File file) throws IOException {
		...
		GraphModelToCsvExporter exporter = new GraphModelToCsvExporter(rows,
				file, chartSettings.getConfigCsvSeparator(), xAxisLabel,
				xAxisLabelRenderer, chartSettings.getHideNonRepValLimit());
	...
	private class SaveAction implements ActionListener {
		...
		public void actionPerformed(final ActionEvent e) {
			JFileChooser chooser = savePath != null ? new JFileChooser(
					new File(savePath)) : new JFileChooser(new File("."));
			...
			if (returnVal == JFileChooser.APPROVE_OPTION) {
				...
				if (file.exists()) {
					int choice = JOptionPane.showConfirmDialog(
							GraphPanelChart.this, "Do you want to overwrite "
				...
				if (doSave) {
					try {
						...
						JOptionPane.showConfirmDialog(GraphPanelChart.this,
								"Impossible to write the image to the file:\n"
										+ ex.getMessage(), "Save Image as",
				...
				if (doSave) {
					try {
						...
						JOptionPane.showConfirmDialog(
								...
								"Impossible to write the CSV file:\n"
										+ ex.getMessage(),
								"Export to CSV File",
								JOptionPane.DEFAULT_OPTION,
								JOptionPane.ERROR_MESSAGE);
