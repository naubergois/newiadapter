...
import br.unifor.iadapter.jmeter.SampleResult;
import br.unifor.iadapter.jmeter.SampleSaveConfiguration;
import br.unifor.iadapter.jmeter.SaveService;
import br.unifor.iadapter.jmeter.StatisticalSampleResult;
import br.unifor.iadapter.jmeter.Visualizer;
...
public final class CSVSaveService {
	...
	private static final String DATE_FORMAT_STRINGS[] = {
			...
			"MM/dd/yy HH:mm:ss" // $NON-NLS-1$ (for compatibility, this is the
								// original default)
	};
		...
		try {
			if (saveConfig.saveTimestamp()) {
				...
				if (saveConfig.printMilliseconds()) {
					try {
					...
					} catch (NumberFormatException e) { // it did not, let's try
														// some other formats
						log.warn(e.toString());
						...
						for (String fmt : DATE_FORMAT_STRINGS) {
							...
							try {
								...
								// method is only ever called from one thread at
								// a time
						...
						if (!foundMatch) {
							throw new ParseException(
									"No date-time format found matching "
											+ text, -1);
						}
					}
	...
	/*
	 * Class to handle generating the delimited string. - adds the delimiter if
	 * not the first call - quotes any strings that require it
	 */
	...
	private enum ParserState {
		INITIAL, PLAIN, QUOTED, EMBEDDEDQUOTE
	}
