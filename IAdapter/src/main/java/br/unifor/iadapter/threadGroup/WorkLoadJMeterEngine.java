...
/**
 * Runs JMeter tests, either directly for local GUI and non-GUI invocations, or
 * started by {@link RemoteJMeterEngineImpl} when running in server mode.
 */
public class WorkLoadJMeterEngine extends
		org.apache.jmeter.engine.StandardJMeterEngine implements JMeterEngine,
		Runnable {

	private static final Logger log = LoggingManager.getLoggerForClass();

	// Should we exit at end of the test? (only applies to server, because host
	// is non-null)
	private static final boolean exitAfterTest = JMeterUtils.getPropDefault(
			"server.exitaftertest", false); // $NON-NLS-1$

	private static final boolean startListenersLater = JMeterUtils
			.getPropDefault("jmeterengine.startlistenerslater", true); // $NON-NLS-1$
	...
	private volatile static WorkLoadJMeterEngine engine;

	/*
	 * Allow functions etc to register for testStopped notification. Only used
	 * by the function parser so far. The list is merged with the testListeners
	 * and then cleared.
	 */
	...
	private static final boolean REMOTE_SYSTEM_EXIT = JMeterUtils
			.getPropDefault("jmeterengine.remote.system.exit", false);

	/** Whether to call System.exit(1) if threads won't stop */
	private static final boolean SYSTEM_EXIT_ON_STOP_FAIL = JMeterUtils
			.getPropDefault("jmeterengine.stopfail.system.exit", true);

	/** Whether to call System.exit(0) unconditionally at end of non-GUI test */
	private static final boolean SYSTEM_EXIT_FORCED = JMeterUtils
			.getPropDefault("jmeterengine.force.system.exit", false);

	/**
	 * Flag to show whether test is running. Set to false to stop creating more
	 * threads.
	 */
	...
	public WorkLoadJMeterEngine() {
		this(null);
	}

	public WorkLoadJMeterEngine(String host) {
		this.host = host;
	...
	public void configure(HashTree testTree) {
		...
		SearchByClass<JMeterTreeNode> testPlan = new SearchByClass<JMeterTreeNode>(
				JMeterTreeNode.class);
		testTree.traverse(testPlan);
		Object[] plan = testPlan.getSearchResults().toArray();
		if (plan.length == 0) {
			throw new RuntimeException("Could not find the TestPlan class!");
		}
		TestPlan tp = (TestPlan) ((JMeterTreeNode) plan[0]).getTestElement();
	...
	private void notifyTestListenersOfStart(
			SearchByClass<TestStateListener> testListeners) {
	...
	private void notifyTestListenersOfEnd(
		...
		for (TestStateListener tl : testListeners.getSearchResults()) {
			try {
				...
				log.warn(
						"Error encountered during shutdown of " + tl.toString(),
						e);
			}
		}
		if (host != null) {
						...
						JMeterUtils
								.reportErrorToUser(
										JMeterUtils
												.getResString("stopping_test_failed"), //$NON-NLS-1$
										JMeterUtils
												.getResString("stopping_test_title")); //$NON-NLS-1$
					}
	...
	public void run() {
		...
		/*
		 * Ensure that the sample variables are correctly initialised for each
		 * run. TODO is this the best way to do this? should it be done
		 * elsewhere ?
		 */
		...
		try {
			...
			JMeterUtils
					.reportErrorToUser("Error occurred compiling the tree: - see log file");
		 ...
		 */
		SearchByClass<TestStateListener> testListeners = new SearchByClass<TestStateListener>(
				TestStateListener.class); // TL - S&E
		...
		List<?> testLevelElements = new LinkedList<Object>(test.list(test
				.getArray()[0]));
		removeThreadGroups(testLevelElements);

		SearchByClass<SetupThreadGroup> setupSearcher = new SearchByClass<SetupThreadGroup>(
				SetupThreadGroup.class);
		SearchByClass<AbstractThreadGroup> searcher = new SearchByClass<AbstractThreadGroup>(
				AbstractThreadGroup.class);
		SearchByClass<PostThreadGroup> postSearcher = new SearchByClass<PostThreadGroup>(
				PostThreadGroup.class);
		...
		Iterator<SetupThreadGroup> setupIter = setupSearcher.getSearchResults()
				.iterator();
		Iterator<AbstractThreadGroup> iter = searcher.getSearchResults()
				.iterator();
		Iterator<PostThreadGroup> postIter = postSearcher.getSearchResults()
				.iterator();
		...
		if (setupIter.hasNext()) {
			...
			while (running && setupIter.hasNext()) {// for each setup thread
													// group
				AbstractThreadGroup group = setupIter.next();
				groupCount++;
				String groupName = group.getName();
				log.info("Starting setUp ThreadGroup: " + groupCount + " : "
						+ groupName);
				startThreadGroup(group, groupCount, setupSearcher,
						testLevelElements, notifier);
		...
		boolean mainGroups = running; // still running at this point, i.e. setUp
										// was not cancelled
		while (running && iter.hasNext()) {// for each thread group
			...
			startThreadGroup(group, groupCount, searcher, testLevelElements,
					notifier);
			if (serialized && iter.hasNext()) {
				log.info("Waiting for thread group: " + groupName
						+ " to finish before starting next group");
				group.waitThreadsStopped();
			}
		...
		if (postIter.hasNext()) {
			...
			while (running && postIter.hasNext()) {// for each setup thread
				...
				groupCount++;
				String groupName = group.getName();
				log.info("Starting tearDown ThreadGroup: " + groupCount + " : "
						+ groupName);
				startThreadGroup(group, groupCount, postSearcher,
						testLevelElements, notifier);
	...
	private void startThreadGroup(AbstractThreadGroup group, int groupCount,
			SearchByClass<?> searcher, List<?> testLevelElements,
		...
		WorkLoadThreadGroup group1 = (WorkLoadThreadGroup) group;

		group1.start(groupCount, notifier, threadGroupTree, this);
	}
	 ...
	 * {@link AbstractThreadGroup#tellThreadsToStop()}
	 */
	...
	public void exit() {
		ClientJMeterEngine.tidyRMI(log); // This should be enough to allow
											// server to exit.
		if (REMOTE_SYSTEM_EXIT) { // default is false
			log.warn("About to run System.exit(0) on " + host);
			// Needs to be run in a separate thread to allow RMI call to return
			// OK
			Thread t = new Thread() {
				@Override
				public void run() {
