...
public class AcoFlowShopWithIsula {
	...
	private static Logger logger = Logger.getLogger(AcoFlowShopWithIsula.class.getName());
	...
	public static void main(String... args) {
		...
		/*try {
			...
			solver.addDaemonActions(new StartPheromoneMatrix<Integer, FlowShopEnvironment>(),
					new FlowShopUpdatePheromoneMatrix());
			solver.getAntColony().addAntPolicies(new PseudoRandomNodeSelection<Integer, FlowShopEnvironment>(),
	...
	private static void showSolution(final double[][] graph,
			final AcoProblemSolver<Integer, FlowShopEnvironment> problemSolver) throws ClassNotFoundException,
	 ...
	 * @throws IOException
	 */
	public static double[][] getTaillardProblemFromFile(String fileName, int numberOfJobs, int numberOfMachines)
			throws IOException {
	 ...
	 */
	public static double[][] getProblemGraphFromFile(String path) throws IOException {
		double[][] graph = null;
		...
		while ((line = buf.readLine()) != null) {
			if (index > 0) {
				...
				logger.info("numberOfJobs=" + numberOfJobs + ", numberOfMachines=" + numberOfMachines);

				if (graph == null) {
					graph = new double[Integer.parseInt(numberOfJobs)][Integer.parseInt(numberOfMachines)];
				}
			}
			index++;
		}
