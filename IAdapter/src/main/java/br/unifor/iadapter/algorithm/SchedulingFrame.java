...
public class SchedulingFrame extends javax.swing.JFrame {
	...
	private void initComponents() {
		...
		layout.setHorizontalGroup(
				layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING).addGap(0, 1330, Short.MAX_VALUE));
		layout.setVerticalGroup(
				layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING).addGap(0, 539, Short.MAX_VALUE));
			...
			for (int i = 0; i < machines; i++) {
				...
				if (i == 0) {
					if (posX == 20) {
						...
						graphics.fillRect(i * (int) machinesTime[i] / factor, posY, (int) tiempo / factor, height);
						posX = 40;
					} else {
						machinesTime[i] = machinesTime[i] + tiempo;
						graphics.fillRect((int) (machinesTime[i] - tiempo) / factor, posY, (int) tiempo / factor,
								height);
					}
				} else {
					if (machinesTime[i] > machinesTime[i - 1]) {
						graphics.fillRect((int) (machinesTime[i]) / factor, posY, (int) tiempo / factor, height);
						machinesTime[i] = machinesTime[i] + tiempo;
					} else {
						machinesTime[i] = machinesTime[i - 1] + tiempo;
						graphics.fillRect((int) machinesTime[i - 1] / factor, posY, (int) tiempo / factor, height);
					}
				}
			}
		}
