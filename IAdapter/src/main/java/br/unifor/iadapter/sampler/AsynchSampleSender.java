...
package br.unifor.iadapter.sampler;
...
import br.unifor.iadapter.jmeter.RemoteSampleListener;
import br.unifor.iadapter.jmeter.SampleEvent;

/**
 * Sends samples in a separate Thread and in Batch mode
 */
public class AsynchSampleSender extends AbstractSampleSender implements
		Serializable {
	...
	private static final int serverConfiguredCapacity = JMeterUtils
			.getPropDefault("asynch.batch.queue.size", DEFAULT_QUEUE_SIZE); // $NON-NLS-1$

	private final int clientConfiguredCapacity = JMeterUtils.getPropDefault(
			"asynch.batch.queue.size", DEFAULT_QUEUE_SIZE); // $NON-NLS-1$

	// created by client
	private final RemoteSampleListener listener;

	private transient BlockingQueue<SampleEvent> queue; // created by server in
														// readResolve method

	private transient long queueWaits; // how many times we had to wait to queue
										// a sample
	...
	private Object readResolve() throws ObjectStreamException {
		...
		log.info("Using batch queue size (asynch.batch.queue.size): "
				+ capacity); // server log file
	...
	private int getCapacity() {
		return isClientConfigured() ? clientConfiguredCapacity
				: serverConfiguredCapacity;
	}
		...
		public void run() {
			try {
				...
				while (!eof) {
					...
					while (!(eof = (e == FINAL_EVENT)) && e != null) { // try to
																		// process
																		// as
																		// many
																		// as
